...
public class MovementCharacter : MonoBehaviour
{
    ...
    private void Update()
    {
        if (GameManager.Instance.State != GameManager.GameState.RoamingMap)
        {
        ...
        // If the ray hits something with navmeshLayer
        if (Physics.Raycast(ray, out hit, Mathf.Infinity, navmeshLayer))
        {
            ...
            ParticleSystem rippleEffectInstance = Instantiate(
                rippleEffectPrefab,
                hit.point,
                Quaternion.identity
            );
